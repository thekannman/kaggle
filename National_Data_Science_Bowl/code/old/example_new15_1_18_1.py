...
for train, test in kf:
	...
	i = max_depth-1
	X_train, X_test, y_train, y_test = X[train,:], X[test,:], y_all[i][train], y_all[i][test] #I think this needs to have split of y_all restructured for upper categories.
	clf = RF(n_estimators=n_estim, n_jobs=3,compute_importances=True)
...
for train, test in kf:
	for i in range(max_depth):
		...
		clf = RF(n_estimators=n_estim, n_jobs=3,compute_importances=True)
		clf.fit(X_train, y_train)
		y_pred[i][test] = clf.predict_proba(X_test)
