...
#train$DISCOUNT_PRICE <- cut(train$DISCOUNT_PRICE,breaks=c(-0.01,0,1000,10000,50000,Inf),labels=c("free","cheap","moderate","expensive","high"))
#convert the factors to columns of 0's and 1's
train <- cbind(train[,c(1,2)],model.matrix(~ -1 + .,train[,-c(1,2)]))

col.means <- colMeans(train[,3:ncol(train)])
col.sd <- apply(train[,3:ncol(train)], 2, sd)
train[,3:ncol(train)] <- apply(train[,3:ncol(train)], 1, function(x) x/col.means)
...
#user.coupons <- matrix(nrow=n.users,ncol=n.coupons)
#rownames(user.coupons) <- users
#colnames(user.coupons) <- coupons
#for (i in 1:nrow(train)) {
#  user.coupons[match(train[i,"USER_ID_hash"],(rownames(user.coupons))),
#               match(train[i,"COUPON_ID_hash"],(colnames(user.coupons)))] <- 1
#}
...
if(only.validate) {
  ...
  uchar.sd <- aggregate(.~USER_ID_hash, data=train[,-1],FUN=sd)
  uchar.sd[is.na(uchar.sd)] <- 0
  #calculation of cosine similairties of users and coupons
  score <- as.matrix(uchar[,2:ncol(uchar)]/(uchar.sd[,2:ncol(uchar)]+0.01)) %*% t(as.matrix(test[,2:ncol(test)]))
