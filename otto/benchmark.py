...
def load_train_as_test_data(path=None):
...
def make_train_submission(clf, encoder, path='rf_train.csv'):
    path = sys.argv[3] if len(sys.argv) > 3 else path
    X_test, ids = load_train_as_test_data()
...
def make_submission(clf, encoder, path='rf_test.csv'):
    path = sys.argv[3] if len(sys.argv) > 3 else path
    X_test, ids = load_data()
    y_prob = clf.predict_proba(X_test)
    with open(path, 'w') as f:
        f.write('id,')
        f.write(','.join(encoder.classes_))
...
def main():
    print(" - Start.")
    clf, encoder = train()
    make_train_submission(clf, encoder)
