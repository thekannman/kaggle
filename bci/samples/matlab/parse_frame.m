function [struct_obj,header,fname] = parse_frame(fname,varargin)


pnames = {'-header','-spec','-skip_lines','-delimiter'};
dflts = {true,'',0,'\t'}; 
...
if ~isempty(arg.spec)
    c = textscan(fid,arg.spec,'Delimiter',arg.delimiter,...
        'Headerlines',arg.skip_lines); 
else
    c = textscan(fid,repmat('%s',[1,num_cols]),'Delimiter',...
        arg.delimiter,'Headerlines',arg.skip_lines); 
end
fclose(fid); 
