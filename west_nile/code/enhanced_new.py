...
def assemble_X(base, weather):
	...
	block = b["Block"]
        ...
        case = [date.year, date.month, date.day, block, lat, longi]
...
def submit(net, mean, std):
    ...
    for i_spec in range(6):
      col_labels.append("species_"+str(i_spec))
    X_file = csv.writer(open("X_test.csv", "w"))
    X_file.writerow(col_labels)
    for row in X:
        X_file.writerow(row)

    id_file = csv.writer(open("test_id.csv", "w"))
    id_file.writerow(["Id"])
    ids = [item['Id'] for item in testing]
    for row in ids:
        id_file.writerow([row])

    predictions = net.predict_proba(X)[:,0]    
    #
    out = csv.writer(open("west_nile_v55044_2.csv", "w"))
