...
country_summary <- ddply(cookies, .(country), .fun=myFun)
...
names(submission.cookie) <- c("device_id","freq")

submissionFile <- paste0("country", format(Sys.time(), "%Y-%m-%d-%H:%M:%S"), ".csv")
