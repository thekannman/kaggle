...
dev_test_basic <- fread("dev_test_basic.csv",stringsAsFactors =FALSE)
cookie_all_basic <- fread("cookie_all_basic.csv",stringsAsFactors =FALSE)
...
write.csv(submission_country_popular_cookie,file="submission_country_popular_cookie.csv",row.names=FALSE)
